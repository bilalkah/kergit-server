...
bool ChatServerState::createChannel(const std::string& name, const HubId& hub_id) {
...
bool ChatServerState::destroyChannel(const std::string& name, const UserId& requester_id) {
...
bool ChatServerState::joinChannel(const UserId& user_id, const std::string& channel_name) {
...
bool ChatServerState::leaveChannel(const UserId& user_id) {
    // auto user_it = users.find(user_id);
...
bool ChatServerState::sendMessage(const UserId& user_id, const std::string& text) {
    // auto user_it = users.find(user_id);
    // if (user_it == users.end()) return false;
    // std::string channel = user_it->second.current_channel;
    // if (channel.empty()) return false;
