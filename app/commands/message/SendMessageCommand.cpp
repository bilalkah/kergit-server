...
std::optional<std::vector<MessageAttachment>> parse_attachments(
    ...
    for (const auto& input : cmd.attachments()) {
        if (input.id().empty() || input.storage_key().empty()) return std::nullopt;
        ...
        next.id = input.id();
        next.kind = input.kind() == sercom::protocol::domain::MessageAttachmentKind::
                                        MessageAttachmentKind_IMAGE
...
std::optional<MessageLinkPreview> parse_link_preview(
    ...
    if (raw.url().size() > kMaxLinkPreviewFieldLength || raw.title().size() > kMaxLinkPreviewFieldLength ||
