...
DeleteHubCommand::DeleteHubCommand(ServiceObjects& svc_objs)
    : services_(svc_objs) {}
...
void DeleteHubCommand::execute(CommandContext& ctx) {
    ...
    try {
        ...
        for (const auto& channel : channels) {
            hub_channel_ids.insert(channel.id);
        }

        if (!services_.db_.hubs().deleteHub(*internal_hub, ctx.snapshot.user_id)) {
            output.success = false;
        ...
        for (const auto& channel_id : hub_channel_ids) {
            const auto subs = services_.gateway_.subscribers(channel_topic(channel_id));
            ...
            services_.gateway_.drop_topic(channel_topic(channel_id));
        }
