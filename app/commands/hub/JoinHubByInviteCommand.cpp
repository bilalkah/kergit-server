...
std::vector<net::outbound::OutgoingMessage> JoinHubByInviteCommand::execute(
    ...
    for (const auto& conn : self_conns) {
        ctx.subscription_manager.subscribeConnection(conn, Topic::HubTopic(hub_id));
    }
    const std::unordered_set<GlobalConnId> own_conn_set(self_conns.begin(), self_conns.end());
    ...
    auto* member_upsert =
        hub_delta->add_member_ops()->mutable_upsert()->mutable_state()->mutable_member();
