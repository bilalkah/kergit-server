    ...
    if (auth.has_voice_resume()) {
        ...
        if (scope_opt.has_value()) {
            const auto transfer = ctx.voice_service.try_resume_voice_ownership(
                user_id, scope_opt->hub_id, scope_opt->channel_id,
                auth.voice_resume().resume_id(), new_session_id);
            if (transfer.has_value() &&
                transfer->previous_owner_session != new_session_id) {
                auto old_conns = ctx.session_manager.getSessionIdConnections(
                    transfer->previous_owner_session);
                if (!old_conns.empty()) {
                    ...
                    result.emplace_back(make_outgoing_message(
                        net::outbound::Target::many(std::move(old_conns)),
