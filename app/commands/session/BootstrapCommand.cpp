...
std::string make_voice_self_status(bool connected, bool is_owner,
    ...
    if (connected && is_owner && resume_id.has_value() && !resume_id->empty()) {
...
std::vector<net::outbound::OutgoingMessage> BootstrapCommand::execute(CommandContext& ctx,
    ...
    const bool is_voice_owner = voice_owner.has_value() && *voice_owner == session_id;
    out.emplace_back(make_outgoing_message(
        net::outbound::Target::one(event->conn_id),
        make_voice_self_status(
            voice_channel.has_value(), is_voice_owner, voice_hub_id, voice_channel_id,
            is_voice_owner ? ctx.voice_service.current_resume_id(user_id) : std::nullopt)));
