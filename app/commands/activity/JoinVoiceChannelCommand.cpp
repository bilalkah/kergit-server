...
std::vector<net::outbound::OutgoingMessage> JoinVoiceChannelCommand::execute(
    ...
    if ((taking_over || switching_channel) && old_channel.has_value()) {
        ...
        if (switching_channel) {
            auto requester_conns =
                ctx.session_manager.getSessionIdConnections(requester_session_id);
            if (!requester_conns.empty()) {
                auto disconnected_msg =
                    make_outgoing_message(net::outbound::Target::many(std::move(requester_conns)),
                                          make_voice_self_status_disconnected());
                out.push_back(std::move(disconnected_msg));
            }
        }
    ...
    try {
        ...
        // Do not surface raw service/DB error text to the client.
        out.emplace_back(make_command_error(
    ...
    if (issued.token.empty() || issued.livekit_url.empty()) {
        const std::string reason =
            issued.error_reason.empty() ? "Voice token issuance failed" : issued.error_reason;
        out.emplace_back(
            make_command_error(event->conn_id, env.type(),
                               sercom::protocol::event::CommandErrorCode_INTERNAL_ERROR, reason));
        return out;
    }
