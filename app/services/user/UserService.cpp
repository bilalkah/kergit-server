...
std::unordered_map<UserId, User> UserService::getUsersByIds(const std::vector<UserId>& userIds) {
    ...
    details << "input=" << userIds.size() << " unique=" << seen_ids.size()
            ...
            << " db=1 db_ms=" << db_ms << " fetched=" << fetched_users.size()
...
std::expected<User, UserService::UpdateError> UserService::updateProfile(
    const UserId& userId, const std::optional<std::string>& user_name,
    const std::optional<std::string>& display_name, const std::optional<std::string>& avatar_seed) {
    try {
        User updated = repo_.updateUserProfile(userId, user_name, display_name, avatar_seed);
        cache_->put(updated);
        return updated;
    } catch (const pqxx::unique_violation&) {
        // user_name is the only unique column touched by this update, so a unique
        // violation here always means the requested handle is already taken.
        cache_->invalidate(userId);
        return std::unexpected(UpdateError::DuplicateUsername);
    } catch (...) {
        cache_->invalidate(userId);
