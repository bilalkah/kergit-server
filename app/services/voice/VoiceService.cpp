...
bool decrypt_key_blob(const std::array<unsigned char, 32>& master_key, std::string_view encoded,
    ...
    const auto* ciphertext =
        reinterpret_cast<const unsigned char*>(packed.data() + 1 + kNonceLen);
    const auto* tag = reinterpret_cast<const unsigned char*>(packed.data() + packed.size() - kTagLen);
    ...
    int written = 0;
    if (ciphertext_len > 0 &&
        EVP_DecryptUpdate(ctx.get(), reinterpret_cast<unsigned char*>(out_plaintext.data()), &written,
...
VoiceService::VoiceService(std::string api_key, std::string api_secret,
    ...
    : voice_state_repo_(voice_state_repo),
      ...
      e2ee_key_ttl_(parse_positive_seconds(
          utils::EnvLoader::get_env("VOICE_E2EE_KEY_TTL_SEC", ""), kDefaultE2EEKeyTtl)),
      ...
      reconcile_interval_(parse_reconcile_interval_seconds(
          utils::EnvLoader::get_env("LIVEKIT_RECONCILE_INTERVAL_SEC", ""), kDefaultReconcileInterval)),
      livekit_missing_clear_ttl_(parse_positive_seconds(
          utils::EnvLoader::get_env("LIVEKIT_MISSING_CLEAR_SEC", ""),
    ...
    if (!e2ee_storage_key_ready_) {
        utils::EventLogger::instance().log(
            utils::EventCategory::VOICE, "", "e2ee_storage_master_key_unavailable", 0,
...
VoiceService::JoinVoiceToken VoiceService::join_voice(const ChannelId& channel, const UserId& user,
    ...
    if (!key.has_value()) {
        ...
        if (!empty_state.has_value() || !*empty_state) {
            utils::EventLogger::instance().log(
                utils::EventCategory::VOICE, user.value, "e2ee_key_missing_active_room", 0,
...
void VoiceService::force_local_leave(const UserId& user, const ChannelId& channel,
    ...
    utils::EventLogger::instance().log(utils::EventCategory::VOICE, user.value,
                                       "reconcile_forced_leave", 0,
                                       "channel=" + channel.value + " reason=" +
                                           std::string(reason));
...
bool VoiceService::confirm_channel_remote_missing(const ChannelId& channel,
    ...
    utils::EventLogger::instance().log(
        ...
        0,
        "channel=" + channel.value + " reason=" + std::string(reason) +
            " first_reason=" + result.first_reason +
            " first_seen=" + (result.first_observation ? "1" : "0") +
...
void VoiceService::reconcile_full_state(std::string_view reason) {
    ...
    for (const auto& node : nodes_.list_nodes()) {
        ...
        try {
            ...
            utils::EventLogger::instance().log(utils::EventCategory::VOICE, "",
                                               "reconcile_list_rooms_failed", 0,
                                               "node=" + node.node_id +
                                                   " method=ListRooms endpoint=" +
                                                   node.private_host + " error=" + ex.what());
            continue;
        }
...
void VoiceService::reconcile_channel_state(const ChannelId& channel, std::string_view reason) {
        ...
        try {
            ...
            utils::EventLogger::instance().log(
                ...
                "node=" + node.node_id + " method=ListParticipants endpoint=" +
                    node.private_host + " channel=" + channel.value + " error=" + ex.what());
            continue;
        } catch (...) {
            ++query_summary.failed_queries;
            utils::EventLogger::instance().log(
                utils::EventCategory::VOICE, "", "reconcile_list_participants_failed", 0,
                "node=" + node.node_id + " method=ListParticipants endpoint=" +
                    node.private_host + " channel=" + channel.value + " error=unknown");
            continue;
        }
    ...
    if (remote_by_user.empty()) {
        ...
        utils::EventLogger::instance().log(utils::EventCategory::VOICE, "",
                                           "reconcile_room_missing_remote", 0,
                                           "channel=" + channel.value +
                                               " reason=" + std::string(reason) + query_details);
        if (confirm_channel_remote_missing(channel, reason)) {
    ...
    for (const auto& [user_id, participant] : remote_by_user) {
        ...
        if (pending.has_value() && pending->to_channel == channel) {
            ...
            const bool nonce_match = !pending->intent_nonce.empty() && !metadata.intent_nonce.empty() &&
        ...
        if (!correlation_valid) {
            ...
            utils::EventLogger::instance().log(
                utils::EventCategory::VOICE, user_id.value, "reconcile_untrusted_remote_participant",
                0,
                "channel=" + channel.value + " reason=" + std::string(reason) +
        ...
        if (active_channel.has_value() && *active_channel == channel) {
            const std::string node_id = assigned_node_id.empty() ? synthetic_event.node_id
                                                                  : assigned_node_id;
            if (!node_id.empty()) {
        ...
        for (const auto& room : rooms) {
                ...
                if (!previous_node || previous_node->node_id != *metadata_node_id) {
                    utils::EventLogger::instance().log(
                        ...
                        "channel=" + room.value +
                            ...
                            " assigned_node=" + *metadata_node_id +
                            " source=participant_metadata");
                }
            ...
            for (const auto& p : participants) {
                ...
                const SessionId recovered_session =
                    metadata.app_session_id != 0
                        ? static_cast<SessionId>(metadata.app_session_id)
                        : next_recovery_session_id();

                const bool first_join_in_channel = sessions_.join(room, p.identity, recovered_session);
                (void)sessions_.set_muted(p.identity, false);
                (void)sessions_.set_deafened(p.identity, false);
                persist_voice_join(p.identity, room, false, false);
...
void VoiceService::on_livekit_event(const livekit::webhook::LiveKitEvent& event) {
    ...
    switch (event.type) {
        ...
        case Type::ROOM_FINISHED:
            ...
            utils::EventLogger::instance().log(
                utils::EventCategory::VOICE, "", "room_finished_requires_reconcile", 0,
                "channel=" + event.channel_id.value);
        ...
        case Type::PARTICIPANT_JOINED: {
            ...
            if (!event.node_id.empty() && current_node_id.empty()) {
                ...
                utils::EventLogger::instance().log(
                    utils::EventCategory::VOICE, event.user_id.value,
        ...
        case Type::PARTICIPANT_CONNECTION_ABORTED: {
            const bool node_mismatch =
                !event.node_id.empty() && !current_node_id.empty() &&
            ...
            const auto after_channel = sessions_.user_channel(event.user_id);
            const bool left_channel = before_channel.has_value() && *before_channel == event.channel_id &&
                                      (!after_channel.has_value() ||
                                       *after_channel != event.channel_id);
            if (left_channel) {
...
std::optional<std::string> VoiceService::load_channel_e2ee_key_from_storage(const ChannelId& channel) {
    ...
    try {
        ...
        if (!decrypt_key_blob(e2ee_storage_master_key_, *raw, decrypted_key)) {
            utils::EventLogger::instance().log(
                utils::EventCategory::VOICE, "", "e2ee_key_decrypt_failed", 0,
                "channel=" + channel.value);
            return std::nullopt;
        }
...
bool VoiceService::persist_channel_e2ee_key_to_storage(const ChannelId& channel,
    ...
    try {
        ...
        if (!encrypt_key_blob(e2ee_storage_master_key_, key, encrypted_blob)) {
            utils::EventLogger::instance().log(
                utils::EventCategory::VOICE, "", "e2ee_key_encrypt_failed", 0,
                "channel=" + channel.value);
            return false;
        }
...
void VoiceService::force_channel_rekey(const ChannelId& channel, std::string_view reason) {
    ...
    if (!node) {
        utils::EventLogger::instance().log(utils::EventCategory::VOICE, "",
                                           "e2ee_forced_rekey", 0,
                                           "channel=" + channel.value +
                                               " removed_participants=0 reason=" +
                                               std::string(reason) + " node=unknown");
        return;
    }
    ...
    utils::EventLogger::instance().log(
        ...
        "channel=" + channel.value + " node=" + node->node_id +
            " removed_participants=" + std::to_string(removed_count) +
...
std::string VoiceService::resume_key(const UserId& user) {
