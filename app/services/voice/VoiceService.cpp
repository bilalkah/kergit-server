      ...
      token_service_(std::move(api_key), std::move(api_secret))
{
    ...
    const auto node1_public = utils::EnvLoader::get_env(
        "LIVEKIT_NODE1_PUBLIC_URL", public_base + "/livekit-node-1");
    const auto node2_public = utils::EnvLoader::get_env(
        "LIVEKIT_NODE2_PUBLIC_URL", public_base + "/livekit-node-2");
...
void VoiceService::persist_voice_join(const UserId& user, const ChannelId& channel)
{
    ...
    try {
        ...
        utils::EventLogger::instance().log(
            utils::EventCategory::VOICE, user.value, "persist_join_failed", 0,
...
void VoiceService::persist_voice_leave(const UserId& user)
{
    try {
        ...
        utils::EventLogger::instance().log(
            utils::EventCategory::VOICE, user.value, "persist_leave_failed", 0,
...
void VoiceService::recover_from_restart()
{
    ...
    try {
        ...
        utils::EventLogger::instance().log(
            utils::EventCategory::VOICE, "", "recovery_db_failed", 0,
            std::string("error=") + ex.what());
    }
    ...
    for (const auto& node : nodes) {
        ...
        try {
            ...
            utils::EventLogger::instance().log(
                utils::EventCategory::VOICE, "", "recovery_list_rooms_failed", 0,
        ...
        for (const auto& room : rooms) {
            ...
            try {
                ...
                utils::EventLogger::instance().log(
                    utils::EventCategory::VOICE, "", "recovery_list_participants_failed", 0,
                    "room=" + room.value + " error=" + ex.what());
                continue;
            }

            utils::EventLogger::instance().log(
                utils::EventCategory::VOICE, "", "recovery_kicking_participants", 0,
