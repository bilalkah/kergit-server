...
bool VoiceSessionManager::transfer_owner_session(const UserId& user,
                                                 const SessionId& expected_owner_session,
                                                 const SessionId& next_owner_session) {
    std::lock_guard lock(mutex_);
    auto it = user_to_channel_.find(user);
    if (it == user_to_channel_.end()) return false;
    if (it->second.owner_session != expected_owner_session) return false;
    it->second.owner_session = next_owner_session;
    return true;
}
...
std::uint64_t VoiceSessionManager::active_voice_user_count() const {
    std::lock_guard lock(mutex_);
    return static_cast<std::uint64_t>(user_to_channel_.size());
}

std::vector<ChannelId> VoiceSessionManager::active_channels() const {
    std::lock_guard lock(mutex_);
