...
class VoiceSessionManager {
   public:
    ...
    /// Transfer voice ownership for a user to a new session when the expected owner matches.
    /// Returns true when ownership is updated.
    bool transfer_owner_session(const UserId& user, const SessionId& expected_owner_session,
    ...
    /// Returns active voice participant count across all channels (unique users).
    std::uint64_t active_voice_user_count() const;

    /// Returns currently active voice channels.
    std::vector<ChannelId> active_channels() const;
