    ...
    while (!st.stop_requested()) {
        ...
        try {
            ...
            utils::EventLogger::instance().log(
                utils::EventCategory::SESSION, "", "ASYNC_MESSAGE_WRITE_FAILED", 0,
                "id:" + req.msg.id.value + ",channel_id:" + req.msg.ch_id.value +
                    ",sender_id:" + req.msg.sender_id.value +
                    ",content_len:" + std::to_string(req.msg.text.size()) + ",attachments:" +
                    std::to_string(req.msg.attachments.size()) + ",error:" + ex.what());
        }
    }
}
