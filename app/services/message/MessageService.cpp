...
std::expected<std::vector<Message>, MessageService::MessageError> MessageService::fetchMessages(
    ...
    try {
        ...
        details << "channel_id=" << channelId.value << " limit=" << limit
                << " db=1 fetched=" << messages.size() << " total_ms=" << elapsed_ms(started_at);
...
MessageService::fetchMessagesAfter(const ChannelId& channelId, const MessageCursor& after,
                                   int limit) {
    const auto started_at = std::chrono::steady_clock::now();
    try {
        ...
        details << "channel_id=" << channelId.value << " after_id=" << after.message_id.value
                << " after_us=" << after.created_at_unix_us << " limit=" << limit
                << " db=1 fetched=" << messages.size() << " total_ms=" << elapsed_ms(started_at);
...
MessageService::fetchMessagesBefore(const ChannelId& channelId, const MessageCursor& before,
    ...
    try {
        ...
        details << "channel_id=" << channelId.value << " before_id=" << before.message_id.value
                << " before_us=" << before.created_at_unix_us << " limit=" << limit
                << " db=1 fetched=" << messages.size() << " total_ms=" << elapsed_ms(started_at);
...
std::expected<Message, MessageService::MessageError> MessageService::sendMessage(
    ...
    std::vector<MessageAttachment> attachments, std::optional<MessageLinkPreview> link_preview) {
    const auto started_at = std::chrono::steady_clock::now();
    try {
        if (async_writer_) {
            Message msg;
            ...
            msg.link_preview = std::move(link_preview);
        ...
        auto msg =
            repo_.sendMessage(channelId, senderId, content, std::move(attachments), link_preview);
        std::ostringstream details;
        details << "channel_id=" << channelId.value << " sender_id=" << senderId.value
