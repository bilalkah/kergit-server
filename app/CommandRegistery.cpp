...
void register_all(Dispatcher& d, PersistenceGateway& db, net::ClientGateway& gateway,
    ...
    d.register_cmd("delete_channel", std::make_unique<DeleteChannelCommand>(db, gateway, connections, hub_pub, ids));
    ...
    d.register_cmd("rename_hub", std::make_unique<RenameHubCommand>(db, gateway, connections,
                                                                    hub_pub, ids));
    d.register_cmd("delete_hub",
                   std::make_unique<DeleteHubCommand>(db, gateway, connections, hub_pub, ids));
    d.register_cmd("generate_hub_invite",
                   std::make_unique<GetHubInviteCommand>(db, ids));
    d.register_cmd("join_hub_by_code",
                   std::make_unique<JoinHubByInviteCommand>(db, gateway, connections, hub_pub,
                                                            ids));
    d.register_cmd("leave_hub", std::make_unique<LeaveHubCommand>(db, gateway, connections,
                                                                  hub_pub, ids));
    d.register_cmd("update_member_role",
                   std::make_unique<UpdateMemberRoleCommand>(db, gateway, connections, ids,
                                                             hub_pub));
