...
TEST_F(EnvLoaderTest, GetEnvListParsesBracketedAndQuotedValues) {
    TempEnvFile t;
    {
        std::ofstream out(t.path);
        out << "PORTS=[7880, 7890]\n";
        out << "HOSTS=['node-1', \"node-2\"]\n";
        out << "EMPTY=[]\n";
    }

    EXPECT_TRUE(EnvLoader::load_env_file(t.path.string()));

    const auto ports = EnvLoader::get_env_list("PORTS");
    ASSERT_EQ(ports.size(), 2U);
    EXPECT_EQ(ports[0], "7880");
    EXPECT_EQ(ports[1], "7890");

    const auto hosts = EnvLoader::get_env_list("HOSTS");
    ASSERT_EQ(hosts.size(), 2U);
    EXPECT_EQ(hosts[0], "node-1");
    EXPECT_EQ(hosts[1], "node-2");

    const std::vector<std::string> defaults{"fallback"};
    EXPECT_EQ(EnvLoader::get_env_list("EMPTY", defaults), defaults);
}
...
TEST_F(EnvLoaderTest, SetEnvAndClearEnvWork) {
    ...
    EXPECT_FALSE(EnvLoader::has_env("X"));
    EnvLoader::set_env("X", "123");
    EnvLoader::set_env("Y", "");
    EXPECT_TRUE(EnvLoader::has_env("X"));
    EXPECT_TRUE(EnvLoader::has_env("Y"));
    EXPECT_EQ(EnvLoader::get_env("X"), "123");
    EXPECT_EQ(EnvLoader::get_env("Y"), "");
    EnvLoader::clear_env();
    EXPECT_FALSE(EnvLoader::has_env("X"));
