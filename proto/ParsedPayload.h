...
using ParsedPayload = std::variant<std::monostate, command::Authenticate,
                                   ...
                                   command::ValidateHubInvite,
                                   command::LeaveHub, command::RemoveHub,
                                   command::KickHubMember, command::UpdateHub,
                                   command::UpdateHubMemberRole, command::CreateChannel,
...
inline ParsedPayloadResult parse_inbound_payload(const Envelope& env) {
    ...
    switch (env.type()) {
        ...
        case Envelope::VOICE_KICK_PARTICIPANT:
            return detail::parse_payload_as<command::KickVoiceParticipant>(
                env, "Invalid VOICE_KICK_PARTICIPANT payload");
        ...
        case Envelope::HUB_INVITE_VALIDATE:
            return detail::parse_payload_as<command::ValidateHubInvite>(
                env, "Invalid HUB_INVITE_VALIDATE payload");
        ...
        case Envelope::HUB_MEMBER_KICK:
            return detail::parse_payload_as<command::KickHubMember>(
                env, "Invalid HUB_MEMBER_KICK payload");
        case Envelope::HUB_UPDATE:
            return detail::parse_payload_as<command::UpdateHub>(env, "Invalid HUB_UPDATE payload");
        case Envelope::HUB_MEMBER_ROLE_UPDATE:
            return detail::parse_payload_as<command::UpdateHubMemberRole>(
                env, "Invalid HUB_MEMBER_ROLE_UPDATE payload");
