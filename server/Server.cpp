#include "server/Server.h"

#include "utils/EnvLoader.h"
#include "utils/EventLogger.h"
#include "utils/Metrics.h"

namespace server {

Server::Server(const core::ServerConfig& config)
    : config_(config), app_stack_(config_), control_http_(config_.control) {}

void Server::init_stacks() {
    if (!config_.socket_ports.empty()) {
        for (size_t i = 0; i < config_.socket_ports.size(); ++i) {
            auto cfg = config_.network;
            cfg.port = config_.socket_ports[i];
            cfg.port_index = i;
            auto net_stack = std::make_unique<net::NetworkStack>(cfg);
            net_stack->attach_event_sink(app_stack_.event_sink());
            net_stack->attach_online_user_count_provider(
                [this]() { return app_stack_.online_user_count(); });
            net_stack->attach_active_webrtc_user_count_provider(
                [this]() { return app_stack_.active_webrtc_user_count(); });
            network_router_.register_stack(std::move(net_stack));
        }
    } else {
        for (size_t i = 0; i < config_.network.socket_threads; ++i) {
            auto net_stack = std::make_unique<net::NetworkStack>(config_.network);
            net_stack->attach_event_sink(app_stack_.event_sink());
            net_stack->attach_online_user_count_provider(
                [this]() { return app_stack_.online_user_count(); });
            net_stack->attach_active_webrtc_user_count_provider(
                [this]() { return app_stack_.active_webrtc_user_count(); });
            network_router_.register_stack(std::move(net_stack));
        }
    }
    app_stack_.attach_outbound_sink(network_router_);
}

void Server::start() {
    // Initialize event logger with path from LOG_DIR env var
    auto log_dir = utils::EnvLoader::get_env("LOG_DIR", "logs");
    utils::EventLogger::instance().init(log_dir);

    init_stacks();
    app_stack_.bootstrap();
    app_stack_.start();
    utils::metrics::start_timeseries();
    network_router_.start_all();
    control_http_.start();
}

void Server::stop() {
    control_http_.stop();
    app_stack_.stop();
    network_router_.stop_all();
    utils::metrics::stop_timeseries();

    // Shutdown event logger
    utils::EventLogger::instance().shutdown();
}

};  // namespace server
