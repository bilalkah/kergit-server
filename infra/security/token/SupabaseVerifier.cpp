...
std::expected<SupabaseVerifier, JwtVerifyError> SupabaseVerifier::create(
    ...
    return SupabaseVerifier{std::move(keys), std::move(key_order), std::move(expected_issuer),
                            utils::EnvLoader::get_env("SUPABASE_EXPECTED_AUD", "")};
}
SupabaseVerifier::SupabaseVerifier(
    std::unordered_map<std::string, KeyMaterial, StringHash, StringEq> keys,
    std::vector<std::string> key_order, std::string expected_issuer, std::string expected_audience)
    : keys_(std::move(keys)),
      key_order_(std::move(key_order)),
...
JwtVerifyResult SupabaseVerifier::verify_token(std::string_view token) const {
    ...
    try {
        ...
        if (!expected_issuer_.empty() && claims.iss != expected_issuer_) {
            return std::unexpected(JwtVerifyError::IssuerMismatch);
        }
        if (!expected_audience_.empty() && claims.aud != expected_audience_) {
