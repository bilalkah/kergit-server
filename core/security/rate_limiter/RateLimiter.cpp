...
void RateLimiter::block_client(const std::string& client_id, int duration_seconds) {
    ...
    limit_info.block_until = std::chrono::system_clock::now() + 
                            std::chrono::seconds(safe_duration);
}
...
bool RateLimiter::is_client_blocked(const std::string& client_id) {
    ...
    if (limit_info.is_blocked && 
        std::chrono::system_clock::now() >= limit_info.block_until) {
...
void RateLimiter::cleanup_expired_entries() {
    ...
    for (auto it = client_limits_.begin(); it != client_limits_.end();) {
        if (!it->second.is_blocked && 
            it->second.last_request < cleanup_threshold) {
...
bool RateLimiter::check_rate_limit(RateLimitInfo& limit_info, int max_requests, int window_seconds) {
    auto now = std::chrono::system_clock::now();
