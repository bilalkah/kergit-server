...
std::string JWTManager::generate_token(const JWTClaims& claims) {
    ...
    json payload = {
        ...
        {"iat",
         std::chrono::duration_cast<std::chrono::seconds>(claims.issued_at.time_since_epoch())
             .count()},
        {"exp",
...
void JWTManager::rotate_secret() {
    ...
    std::string new_secret =
        "rotated_secret_" + std::to_string(std::chrono::duration_cast<std::chrono::seconds>(
