...
int ChatDB::createHub(const std::string& hubName, int ownerId) {
    pqxx::work txn(conn_);
    auto result = txn.exec_params("INSERT INTO hubs (name, owner_id) VALUES ($1, $2) RETURNING id",
                                  hubName, ownerId);
    int hubId = result[0][0].as<int>();

    txn.exec_params("INSERT INTO hub_members (hub_id, user_id, role) VALUES ($1, $2, 'owner')",
                    hubId, ownerId);
...
void ChatDB::addMember(int hubId, int userId) {
    pqxx::work txn(conn_);
    txn.exec_params(
        "INSERT INTO hub_members (hub_id, user_id, role) VALUES ($1, $2, 'member') ON CONFLICT DO "
        "NOTHING",
        hubId, userId);
    txn.commit();
}
...
void ChatDB::removeMember(int hubId, int userId) {
    pqxx::work txn(conn_);
    txn.exec_params("DELETE FROM hub_members WHERE hub_id = $1 AND user_id = $2", hubId, userId);
    txn.commit();
}
...
int ChatDB::createChannel(int hubId, const std::string& channelName, const std::string& type) {
    ...
    auto result = txn.exec_params(
        "INSERT INTO channels (hub_id, name, type) VALUES ($1, $2, $3) RETURNING id", hubId,
        channelName, type);
...
void ChatDB::sendMessage(int channelId, int senderId, const std::string& content) {
    pqxx::work txn(conn_);
    txn.exec_params("INSERT INTO messages (channel_id, sender_id, content) VALUES ($1, $2, $3)",
...
std::vector<HubInfo> ChatDB::getUserHubs(int userId) {
    pqxx::work txn(conn_);
    auto res = txn.exec_params(
        "SELECT h.id, h.name FROM hubs h JOIN hub_members m ON h.id = m.hub_id WHERE m.user_id = "
        "$1 ORDER BY h.id",
        userId);
...
std::vector<ChannelInfo> ChatDB::getHubChannels(int hubId) {
    pqxx::work txn(conn_);
    auto res = txn.exec_params(
        "SELECT id, hub_id, name, type FROM channels WHERE hub_id = $1 ORDER BY id", hubId);
...
int ChatDB::ensurePersonalHubWithGeneral(int userId, const std::string& hubName) {
    ...
    auto res =
        txn.exec_params("SELECT id FROM hubs WHERE owner_id = $1 ORDER BY id LIMIT 1", userId);
    int hubId;
    if (res.empty()) {
        auto r2 = txn.exec_params("INSERT INTO hubs (name, owner_id) VALUES ($1, $2) RETURNING id",
                                  hubName, userId);
        hubId = r2[0][0].as<int>();
        txn.exec_params("INSERT INTO hub_members (hub_id, user_id, role) VALUES ($1, $2, 'owner')",
                        hubId, userId);
        ...
        txn.exec_params(
            "INSERT INTO hub_members (hub_id, user_id, role) VALUES ($1, $2, 'member') ON CONFLICT "
            "DO NOTHING",
            hubId, userId);
    }
    ...
    auto ch =
        txn.exec_params("SELECT id FROM channels WHERE hub_id = $1 AND name = 'general'", hubId);
    if (ch.empty()) {
        txn.exec_params("INSERT INTO channels (hub_id, name, type) VALUES ($1, 'general', 'text')",
                        hubId);
    }
