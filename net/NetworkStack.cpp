...
void NetworkStack::attach_online_user_count_provider(
    transport::websocket::TextWSServer::OnlineUsersProvider provider) {
    online_user_count_provider_ = std::move(provider);
}

void NetworkStack::attach_active_webrtc_user_count_provider(
    transport::websocket::TextWSServer::ActiveWebRtcUsersProvider provider) {
    active_webrtc_user_count_provider_ = std::move(provider);
}
...
void NetworkStack::wire_components() {
    ...
    // Transport later
    auto ws_origin_policy = security::transport::WsOriginPolicy::from_file(
        cfg_.ws_origin_policy_path, cfg_.public_origin);
    transport::websocket::WsLimits ws_limits{
        .max_message_bytes = cfg_.max_message_size,
        .max_connections = cfg_.max_connections,
    };
    auto ws_transport = std::make_unique<transport::websocket::TextWSServer>(
        cfg_, *connection_registry_, *outgoing_queue_, std::move(ws_origin_policy),
        std::move(ws_limits));
    ws_transport->set_online_users_provider(online_user_count_provider_);
    ws_transport->set_active_webrtc_users_provider(active_webrtc_user_count_provider_);
    transport_layer_ = std::move(ws_transport);
