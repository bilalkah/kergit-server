...
class TextWSServer : public ITransportServer,
   ...
   public:
    using OnlineUsersProvider = std::function<std::uint64_t()>;
    using ActiveWebRtcUsersProvider = std::function<std::uint64_t()>;
    ...
    void set_online_users_provider(OnlineUsersProvider provider);
    void set_active_webrtc_users_provider(ActiveWebRtcUsersProvider provider);
   ...
   private:
    ...
    Hooks hooks_{};
    OnlineUsersProvider online_users_provider_{};
    ActiveWebRtcUsersProvider active_webrtc_users_provider_{};
